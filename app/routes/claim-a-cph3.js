...
const baseURL = 'livestock-back-office/claim-cph/v3'
...
const CLAIMANT_ROLES = {
  'cph-holder': { serviceRole: 'keeper-admin', serviceRoleLabel: 'Keeper admin' },
  delegate: { serviceRole: 'keeper', serviceRoleLabel: 'Keeper' },
  agent: { serviceRole: 'agent-admin', serviceRoleLabel: 'Agent Admin' }
}

const DEFAULT_CLAIMANT_ROLE = 'cph-holder'
...
function getClaimantRole (request) {
  const savedRole = request.session.data.claimCphSettings?.claimantRole
  return Object.hasOwn(CLAIMANT_ROLES, savedRole) ? savedRole : DEFAULT_CLAIMANT_ROLE
}
...
function addMatchedCphHolderHoldings (request) {
  if (getClaimantRole(request) !== 'cph-holder') return

  const matchedHoldings = getEmailMatchedHoldings(request)
  const role = CLAIMANT_ROLES['cph-holder']

  matchedHoldings.forEach(holding => {
    addCompletedHolding(request, {
      ...buildPendingHolding(holding, 'matched-contact'),
      registeredToUser: 'yes',
      registeredToUserLabel: 'Yes',
      serviceRole: role.serviceRole,
      serviceRoleLabel: role.serviceRoleLabel,
      verificationMethod: 'matched-email-or-phone',
      verificationOutcome: 'success',
      linkStatus: 'linked',
      linkStatusLabel: 'Linked',
      linkStatusTagColour: 'green',
      autoLinkedFromContact: true
    })
  })
}
