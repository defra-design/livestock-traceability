...
router.get('/'+baseURL+'/cattle', (req, res) => {
  ...
  router.get('/' + baseURL + '/cattle/:earTagNumber', (req, res) => {
    ...
    const animal = cattleData.animals.find(
      (animal) =>
        animal.earTagNumber.toLowerCase() ===
        req.params.earTagNumber.toLowerCase()
    );
    ...
    const offspring = cattleData.animals.filter(
      (record) =>
        record.dam?.geneticDam?.earTagNumber === animal.earTagNumber
    );

    return res.render(baseURL + '/cattle-details', {
      animal,
      offspring
    });
  });
  ...
  router.get('/' + baseURL + '/holdings', (req, res) => {
    ...
    router.get('/' + baseURL + '/holdings/:id', (req, res) => {
      ...
      const holding = holdingsData.holdings.find(
        (holding) => holding.id === req.params.id
      );
