...
const baseURL = 'livestock-back-office/steel-thread/v3';
...
function getOakfieldLivestockData(req) {
  return req.session.data.oakfieldLivestock || { animals: [] };
}
...
function getAnimalByEarTag(req, earTagNumber) {
  ...
  const oakfieldLivestockData = getOakfieldLivestockData(req);

  const matchesEarTag = (animal) => {
    return String(animal.earTagNumber || '').toLowerCase()
      === String(earTagNumber || '').toLowerCase();
  };

  return (
    cattleData.animals.find(matchesEarTag)
    || oakfieldLivestockData.animals.find(matchesEarTag)
  );
}
...
function getHoldingRegisterCattle(req, holding) {
  const search = String(req.query.search || '').trim();
  const searchTerm = normalise(search);

  const isOakfield = (
    normalise(holding.cph) === normalise('12/345/0001')
  );

  const sourceData = isOakfield
    ? getOakfieldLivestockData(req)
    : getCattleData(req);

  const cattle = sourceData.animals.filter((animal) => {
    if (!search) return true;
...
function getHoldingRegisterPageHref(req, page) {
  const params = new URLSearchParams();
...
function getHoldingRegisterPagination(req, totalResults, pageSize) {
  ...
  const pagination = {
    items: Array.from(
      ...
      (_, index) => {
        ...
        return {
          number: page,
          href: getHoldingRegisterPageHref(req, page),
  ...
  if (currentPage > 1) {
    pagination.previous = {
      href: getHoldingRegisterPageHref(
        req,
        currentPage - 1
      )
    };
  ...
  return {
    ...
    startIndex,
    endIndex,
    pageStart: totalResults ? startIndex + 1 : 0,
    pageEnd: endIndex,
    pagination
  };
}
...
router.get('/' + baseURL + '/holdings/:id/cattle-register', (req, res) => {
  ...
  const cattleResults = getHoldingRegisterCattle(
    req,
    holding
  );

  const pageSize = 20;
  const totalResults = cattleResults.cattle.length;

  const pageDetails = getHoldingRegisterPagination(
    req,
    totalResults,
    pageSize
  );

  const cattle = cattleResults.cattle.slice(
    pageDetails.startIndex,
    pageDetails.endIndex
  );
  ...
  return res.render(baseURL + '/holding-cattle-register', {
    ...
    totalResults,
    pageSize,
    pageStart: pageDetails.pageStart,
    pageEnd: pageDetails.pageEnd,
    pagination: pageDetails.pagination,
    holding,
    baseURL
  });
});
